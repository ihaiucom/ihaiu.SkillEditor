	...
	public class Files
	{
		...
		public static Texture2D LoadTextureFromDll(string resourceName, int width, int height)
		{
			...
			if (EditorApp.IsSourceCodeVersion)
			{
				...
				if (texture2D == null)
				{
					Debug.LogError(Strings.get_Could_not_load_resource() + resourceName);
					texture2D = new Texture2D(2, 2);
				}
				return texture2D;
			}
			texture2D = new Texture2D(width, height, 5, false, true);
			...
			if (manifestResourceStream != null)
			{
				...
				{
					texture2D.set_filterMode(0);
				}
				manifestResourceStream.Close();
			}
			else
			{
				Debug.LogWarning(string.Format(Strings.get_Label_Missing_Dll_resource__(), resourceName));
			}
		...
		public static bool CreateFilePath(string fullFileName)
		{
			...
			{
				Debug.LogError(string.Format(Strings.get_Error_Invalid_path__(), fullFileName));
				return false;
			}
			try
			{
			...
			{
				Debug.LogError(string.Format(Strings.get_Error_Could_not_create_directory__(), directoryName));
				return false;
			}
			return true;
		}
			...
			for (int i = 0; i < assemblies.Length; i++)
			{
				...
				try
				{
					...
					for (int j = 0; j < array.Length; j++)
					{
						...
						{
							Files.scriptList.Add(type.get_FullName());
						}
					}
				}
				catch (Exception ex)
