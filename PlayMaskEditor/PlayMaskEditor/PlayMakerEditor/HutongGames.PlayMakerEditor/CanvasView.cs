...
namespace HutongGames.PlayMakerEditor
{
	public class CanvasView
	{
		...
		private Vector2 oldScrollPosition = Vector2.get_zero();
		...
		public Rect ContentArea
		{
			get
			{
				return new Rect(this.viewRect.get_width() - this.margin * 0.5f, this.viewRect.get_height() - this.margin * 0.5f, this.ContentSize.x * this.scale, this.ContentSize.y * this.scale);
			}
		}
		public Rect Canvas
		{
		...
		public Rect ViewRectInCanvas
		{
			get
			{
				return new Rect(this.scrollPosition.x - this.ContentOrigin.x, this.scrollPosition.y - this.ContentOrigin.y, this.View.get_width(), this.View.get_height());
			}
		}
		...
		public Vector2 CanvasSize
		{
			get
			{
				return new Vector2(this.ContentSize.x * this.scale + this.viewRect.get_width() * 2f - this.margin, this.ContentSize.y * this.scale + this.viewRect.get_height() * 2f - this.margin);
			}
		}
		...
		public Vector2 ContentOrigin
		{
			get
			{
				return new Vector2(this.ContentArea.get_x(), this.ContentArea.get_y());
			}
		}
		public Rect View
		{
		...
		public Vector2 ViewOrigin
		{
			get
			{
				return new Vector2(this.viewRect.get_x(), this.viewRect.get_y());
			}
		}
		public Vector2 ViewSize
		{
			get
			{
				return new Vector2(this.viewRect.get_width(), this.viewRect.get_height());
			}
		}
		public Vector2 ViewCenter
		{
			get
			{
				return new Vector2(this.viewRect.get_width() * 0.5f, this.viewRect.get_height() * 0.5f);
			}
		}
		...
		private Vector2 screenshotSourcePosition
		{
			get
			{
				return new Vector2(this.scrollPosition.x - this.ContentArea.get_x(), this.scrollPosition.y - this.ContentArea.get_y());
			}
		}
		...
		public Vector2 BeginGUILayout(Rect view, bool showScrollBars)
		{
			...
			if (showScrollBars)
			{
				this.activeView.set_width(this.activeView.get_width() - CanvasView.verticalScrollbarWidth);
				this.activeView.set_height(this.activeView.get_height() - CanvasView.horizontalScrollbarHeight);
			}
		...
		{
			if (this.parentWindowPosition.get_width() == 0f)
			{
				this.parentWindowPosition = this.parentWindow.get_position();
				return;
			}
			float num = this.parentWindow.get_position().get_width() - this.parentWindowPosition.get_width();
			float num2 = this.parentWindow.get_position().get_height() - this.parentWindowPosition.get_height();
		...
		public bool IsVisible(Rect rect)
		{
			...
			float num2 = num + this.View.get_width() / this.scale;
			float num3 = this.scrollPosition.y - this.ContentOrigin.y;
			float num4 = num3 + this.View.get_height() / this.scale;
			return rect.get_x() <= num2 && rect.get_x() + rect.get_width() >= num && rect.get_y() <= num4 && rect.get_y() + rect.get_height() >= num3;
		}
		...
		public Vector2 DoEdgeScroll()
		{
			...
			{
				if (this.mousePosition.x > this.viewRect.get_width() - this.EdgeScrollZone && this.mousePosition.x < this.viewRect.get_width())
				{
					float num2 = (this.mousePosition.x - (this.viewRect.get_width() - this.EdgeScrollZone)) / this.EdgeScrollZone;
			...
			{
				if (this.mousePosition.y > this.viewRect.get_height() - this.EdgeScrollZone && this.mousePosition.y < this.viewRect.get_height())
				{
					float num4 = (this.mousePosition.y - (this.viewRect.get_height() - this.EdgeScrollZone)) / this.EdgeScrollZone;
		...
		public void TakeScreenshot(string filename)
		{
			...
			this.screenshotCaptureRect.set_width(this.screenshotCaptureRect.get_width() - 1f);
			this.screenshotCaptureRect.set_y(this.parentWindow.get_position().get_height() - this.screenshotCaptureRect.get_height() - this.screenshotCaptureRect.get_y() + 1f);
			this.screenshotCaptureRect.set_height(this.screenshotCaptureRect.get_height() - 1f);
			if (this.isDocked)
			{
				this.screenshotCaptureRect.set_width(this.screenshotCaptureRect.get_width() - 1f);
				this.screenshotCaptureRect.set_height(this.screenshotCaptureRect.get_height() - 1f);
			...
			this.screenshotTargetPosition = new Vector2(0f, Mathf.Max(this.ContentSize.y - this.screenshotCaptureRect.get_height(), 0f));
			this.parentWindow.Repaint();
			this.ViewChanged = true;
		}
		private void CaptureScreenshotTile()
		{
			...
			bool flag = false;
			bool flag2 = false;
			if (this.screenshotSourcePosition.x + this.screenshotCaptureRect.get_width() >= this.ContentArea.get_width())
			{
				rect.set_width(this.ContentArea.get_width() - this.screenshotSourcePosition.x);
				flag = true;
			}
			if (this.screenshotSourcePosition.y + this.screenshotCaptureRect.get_height() >= this.ContentArea.get_height())
			{
				rect.set_height(this.ContentArea.get_height() - this.screenshotSourcePosition.y);
				rect.set_y(this.screenshotCaptureRect.get_y() + this.screenshotCaptureRect.get_height() - rect.get_height());
				if (flag)
				{
					flag2 = true;
				}
			...
			else
			{
				if (flag)
				{
					this.scrollPosition.x = this.ContentArea.get_x();
					this.scrollPosition.y = this.scrollPosition.y + this.screenshotCaptureRect.get_height();
					this.screenshotTargetPosition.x = 0f;
					this.screenshotTargetPosition.y = this.screenshotTargetPosition.y - this.screenshotCaptureRect.get_height();
				...
				{
					this.scrollPosition.x = this.scrollPosition.x + this.screenshotCaptureRect.get_width();
					this.screenshotTargetPosition.x = this.screenshotTargetPosition.x + this.screenshotCaptureRect.get_width();
				}
			}
			this.parentWindow.Repaint();
		...
		{
			return new Vector2(eventPos.x - this.viewRect.get_x(), eventPos.y - this.viewRect.get_y());
		}
		...
		private void HandleInputEvents()
		{
			...
			this.mouseOverView = (this.mousePosition.x < this.viewRect.get_width() && this.mousePosition.y < this.viewRect.get_height());
